...
Stabilize and commit the current compilable Angular RAG page baseline.

## Instructions I reconciled
The compact LP worker did not return its requested model-authored summary. Codex must inspect the exact diff.
...
- Exact task gate exit code: `0`.
- Gate timed out: `no`.
- A green exit code proves only the assertions implemented by the exact gate; Codex must still review the diff and acceptance contract.
