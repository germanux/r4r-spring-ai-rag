...
def load_task(path: Path) -> Task:
    ...
    required = {
        "schema_version", "id", "benchmark", "objective", "allowed_paths",
        "pre_gate", "post_gate", "opencode_prompt", "review_required",
    }
...
def validate_decision(raw: Any, task_id: str) -> dict[str, Any]:
    ...
    if raw["schema_version"] != 1 or raw["task_id"] != task_id:
        raise ValueError("Decision version or task_id is invalid")
