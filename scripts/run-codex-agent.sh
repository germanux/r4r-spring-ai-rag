...
export OPENCODE_CONFIG="$ROOT/$resolved_config" OPENCODE_CONFIG_DIR="$ROOT/.opencode"
...
if (( SMOKE )); then
  echo "OK: endpoint y modelo disponibles para $DEST"
  exit 0
fi
...
command -v "${R4R_OPENCODE_BIN}" >/dev/null 2>&1 || { echo "OpenCode no está en PATH" >&2; exit 2; }
