...
# Snapshot only the selected execution.
...
python3 - "$TEMP_ZIP" "$PACKAGE_NAME" <<'PY'
from pathlib import Path
...
package_name = sys.argv[2]
runtime_prefix = f"{package_name}/runtime/run/"
...
try:
    with ZipFile(archive_path, "r") as archive:
        ...
        if not any(name.startswith(runtime_prefix) for name in names):
            raise SystemExit(
                "ZIP integrity check failed: selected runtime snapshot is missing"
            )
