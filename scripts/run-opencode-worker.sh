...
while (($#)); do
  case "$1" in
    ...
    --skip-endpoint-check)
      ...
      ;;
    --force-lp-task)
      echo "AVISO: --force-lp-task no existe con el plan fullstack canónico" >&2
      shift
      ;;
    *)
      CONTROLLER_ARGS+=("$1")
...
export R4R_LEGACY_PROGRESS_PATH="$ROOT/$legacy_progress"
...
export R4R_OPENCODE_CONTEXT_DELTA_BYTES="${R4R_OPENCODE_CONTEXT_DELTA_BYTES:-$runtime_context_delta_bytes}"
export R4R_OPENCODE_CONTEXT_WARNING_TOKENS="${R4R_OPENCODE_CONTEXT_WARNING_TOKENS:-$runtime_context_warning_tokens}"
export R4R_OPENCODE_CONTEXT_STOP_TOKENS="${R4R_OPENCODE_CONTEXT_STOP_TOKENS:-$runtime_context_stop_tokens}"
...
if [[ ! -f "$ROOT/$memory" && -n "$legacy_memory" && -f "$ROOT/$legacy_memory" ]]; then
  cp -- "$ROOT/$legacy_memory" "$ROOT/$memory"
fi
...
if [[ -n "$base_url" ]] && (( ! SKIP_ENDPOINT )) && [[ "${R4R_OPENCODE_ENDPOINT_CHECK:-true}" == true ]]; then
...
PYTHON="${R4R_PYTHON_BIN:-python3}"
command -v "$PYTHON" >/dev/null 2>&1 || {
  echo "Python 3 no está en PATH" >&2
  exit 2
}
export PYTHONPATH="$ROOT/py-ring-agent/src${PYTHONPATH:+:$PYTHONPATH}"
