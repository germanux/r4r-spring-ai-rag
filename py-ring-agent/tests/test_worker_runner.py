...
class RunnerTest(unittest.TestCase):
    def test_worker_side_surgical_review_is_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            environment["R4R_REQUIRE_SURGICAL_REVIEW"] = "true"
            with patch.dict(os.environ, environment, clear=False):
                with self.assertRaisesRegex(ValueError, "only through Ring"):
                    AutomaticRunner(repo, plan, progress)
    ...
    def test_lock_auto_advance_accepts_canonical_and_packaged_maintenance_paths(self):
        self.assertTrue(
            is_lock_auto_advance_path(
                "py-ring-agent/src/r4r_worker/runner.py"
            )
        )
        self.assertTrue(
            is_lock_auto_advance_path(
                "r4r-agent-update-v2/py-ring-agent/src/r4r_worker/runner.py"
            )
        )
        self.assertTrue(
            is_lock_auto_advance_path(
                "r4r-self-recovery/scripts/run-opencode-worker.sh"
            )
        )
        self.assertTrue(
            is_lock_auto_advance_path(
    ...
    def test_controller_runtime_paths_are_not_product_scope(self):
        self.assertTrue(
            is_controller_runtime_path(
                "runtime/control/ring-extra-instructions.md"
            )
        )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            (control / "ring-extra-instructions.md").write_text(
                "controller state",
                encoding="utf-8",
            )
    ...
    def test_context_watchdog_declares_all_configured_limits(self):
        watchdog = CommandWatchdog(
            max_seconds=5400,
            idle_seconds=900,
            max_steps=30,
            repeat_event_budget=12,
            max_delta_bytes=65536,
            warning_tokens=80000,
            stop_tokens=120000,
        )
        self.assertEqual(watchdog.max_delta_bytes, 65536)
        self.assertEqual(watchdog.warning_tokens, 80000)
        self.assertEqual(watchdog.stop_tokens, 120000)
        self.assertEqual(watchdog.max_steps, 30)
    ...
    def test_instruction_bundle_includes_task_companion_and_codex_extra(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            runner.escalation_extra_instructions_path = (
                runner.control_dir / "ring-extra-instructions.md"
            )
            runner.escalation_extra_instructions_path.write_text("correction", encoding="utf-8")
            task = Task(
                "task-02-ingestion",
            ...
            self.assertIn("runtime/control/ring-extra-instructions.md", relative)
    ...
    def test_codex_revision_writes_extra_instructions_and_accept_clears_them(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            runner._write_escalation_extra_instructions(task, revise)

            content = runner.escalation_extra_instructions_path.read_text(encoding="utf-8")
            ...
            runner._write_escalation_extra_instructions(task, {"decision": "ACCEPT"})
            self.assertFalse(runner.escalation_extra_instructions_path.exists())
    ...
    def test_resume_uses_pending_codex_extra_instructions_for_same_task(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            runner.escalation_extra_instructions_path.write_text(
                "- Active task: `task-02`\n\nFix exact headings.\n",
                encoding="utf-8",
            )
            task = Task("task-02", "task.md", "objective", ("src/**",), ("true",), "commit")
            ...
            self.assertIsNotNone(runner._resume_action_from_escalation_extra(task))
            self.assertIsNone(runner._resume_action_from_escalation_extra(other))
    ...
    def test_failed_assimilation_becomes_reviewable_evidence(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("OpenCode escalation must review", report)
    ...
    def test_unlocked_resume_accepts_downloaded_r4r_artifacts(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            runner._validate_unlocked_resume((
                ...
                "payload/py-ring-agent/runner.py",
                "SHA256SUMS.txt",
            ), task)
    ...
    def test_watchdog_stops_at_context_token_limit(self):
        with tempfile.TemporaryDirectory() as directory:
            script = (
                "import json,time\n"
                "print(json.dumps({'type':'message','usage':{'total_tokens':128}}), flush=True)\n"
                "time.sleep(10)\n"
            )
            result = run_command(
                ("python3", "-c", script),
                Path(directory),
                timeout_seconds=20,
                watchdog=CommandWatchdog(max_seconds=20, stop_tokens=100),
            )

        self.assertEqual(124, result.exit_code)
        self.assertEqual("context-token-limit", result.stop_reason)
        self.assertEqual(128, result.observed_tokens)

    def test_watchdog_stops_at_context_delta_limit(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = run_command(
                ...
                Path(directory),
                timeout_seconds=20,
                watchdog=CommandWatchdog(max_seconds=20, max_delta_bytes=64),
            )

        self.assertEqual(124, result.exit_code)
        self.assertEqual("context-delta-limit", result.stop_reason)
        self.assertGreater(result.largest_delta_bytes, 64)

    def test_whitespace_repair_preserves_partial_staging(self):
        with tempfile.TemporaryDirectory() as directory:
            repo = Path(directory)
            self._init_repo(repo)
            ...
            run_command(("git", "commit", "-q", "-m", "set fixture"), repo)
            tracked.write_text("alpha staged   \nbeta\n", encoding="utf-8")
            run_command(("git", "add", "tracked.txt"), repo)
            tracked.write_text(
                "alpha staged   \nbeta changed   \n",
                encoding="utf-8",
            ...
            repaired = runner._repair_trailing_whitespace(task)

            self.assertEqual(["tracked.txt"], repaired)
            self.assertEqual(0, run_command(("git", "diff", "--check"), repo).exit_code)
            self.assertEqual(
                0,
                run_command(("git", "diff", "--cached", "--check"), repo).exit_code,
            )
            staged = run_command(("git", "diff", "--cached"), repo).stdout
            unstaged = run_command(("git", "diff"), repo).stdout
    ...
    def test_retry_authorization_allows_only_one_v1_to_v2_upgrade(self):
        with tempfile.TemporaryDirectory() as directory:
            repo = Path(directory)
            directive = repo / "assignment.json"
            runner = object.__new__(AutomaticRunner)
            runner.worker_id = "PC"
            runner.ring_directive_path = directive
            runner._write_progress = lambda _task_id: None
            task = Task(
                "task-01",
                "task.md",
                "repair",
            ...
            progress_item = {
                "status": "BLOCKED",
                "recovery_authorization_consumed": "legacy-v1",
                "recovery_grants_total": 1,
                "recovery_repair_policy_version": 1,
            }
            directive.write_text(
                json.dumps(
                    {
            ...
            self.assertEqual(
                "upgrade-v2",
                runner._consume_retry_authorization(task, progress_item),
            )
            self.assertEqual(2, progress_item["recovery_grants_total"])
            self.assertIsNone(runner._consume_retry_authorization(task, progress_item))
            value = json.loads(directive.read_text(encoding="utf-8"))
    ...
    def test_failed_integration_sync_does_not_accept_task(self):
        ...
        runner.progress = {
            ...
            "last_run": None,
            "tasks": [
                {"id": "task-01", "status": "PENDING", "accepted_at": None}
            ],
        }
        runner._commit_if_needed = lambda *_args: "a" * 40
        runner._sync_with_integration = lambda *_args, **_kwargs: False
        runner._finish = lambda _status, code, _data=None: code

        result = runner._finalize_accepted_task(task)
    ...
    def _selection_runner(repo: Path) -> AutomaticRunner:
        ...
        runner.ring_directive_max_age_seconds = 10800
        runner.global_progress_path = None
        runner.ring_directive_path = repo / "runtime" / "control" / "PC" / "assignment.json"
        runner.ring_directive_path.parent.mkdir(parents=True, exist_ok=True)
        now = datetime.now(timezone.utc)
        runner.ring_directive_path.write_text(
            json.dumps(
                {
                    "schema_version": 1,
                    "assignment_id": "test:PC:task-02",
                    "target": "PC",
                    "priority": "advisory",
                    "action": "CONTINUE",
                    "task_id": "task-02",
