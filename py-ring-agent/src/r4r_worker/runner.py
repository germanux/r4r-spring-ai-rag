...
from .contracts import Task, TaskPlan, load_progress, task_progress
...
class CommandWatchdog:
    ...
    max_delta_bytes: int = 0
    warning_tokens: int = 0
    stop_tokens: int = 0


def _opencode_token_count(value: Any) -> int:
    """Return the largest cumulative token count advertised by an event."""
    counts: list[int] = []
    ...
    def visit(item: Any) -> None:
        if isinstance(item, dict):
            for key, nested in item.items():
                ...
                if isinstance(nested, int) and nested >= 0 and "token" in normalized:
...
def _opencode_event_observation(line: str) -> tuple[bool, bool, str, int]:
    """Return step, progress, signature and cumulative tokens for one event."""
    stripped = line.strip()
    if not stripped:
        return False, False, "empty", 0
    try:
    ...
    if not isinstance(value, dict):
        return False, True, "json-non-object", 0
    tokens = _opencode_token_count(value)
    ...
    if normalized_type in {"step_start", "stepstart"}:
        return True, False, "step_start", tokens
    ...
    if tool_name or normalized_type in {"tool", "tool_use", "tooluse"}:
        ...
        return False, True, f"tool:{tool_name or 'unknown'}:{status}:{signature}", tokens
    ...
    if isinstance(text_value, str) and text_value.strip():
        ...
        return False, True, f"text:{signature}", tokens

    if normalized_type in {"step_finish", "stepfinish", "message", "error"}:
        reason = str(part.get("reason") or value.get("reason") or "")
        return False, True, f"{normalized_type}:{reason}", tokens
    return False, True, f"event:{normalized_type}", tokens
...
def run_command(
    ...
    def observe(line: str) -> None:
        if watchdog is None:
            return
        is_step, meaningful, signature, tokens = _opencode_event_observation(line)
        delta_bytes = len(line.encode("utf-8", errors="replace"))
        now = time.monotonic()
        with state_lock:
            if is_step:
                watch_state["steps"] += 1
            watch_state["observed_tokens"] = max(
                int(watch_state["observed_tokens"]),
                tokens,
            )
            watch_state["largest_delta_bytes"] = max(
                int(watch_state["largest_delta_bytes"]),
                delta_bytes,
            )
            if meaningful:
            ...
            if watchdog.max_delta_bytes > 0 and delta_bytes > watchdog.max_delta_bytes:
                watch_state["stop_reason"] = "context-delta-limit"
            if (
                watchdog.stop_tokens > 0
                and int(watch_state["observed_tokens"]) >= watchdog.stop_tokens
            ):
                watch_state["stop_reason"] = "context-token-limit"
            if (
                watchdog.warning_tokens > 0
                and int(watch_state["observed_tokens"]) >= watchdog.warning_tokens
                and not watch_state["warned_tokens"]
            ):
                watch_state["warned_tokens"] = True
                print(
                    "[r4r-watchdog] context warning: "
                    f"tokens={watch_state['observed_tokens']} "
                    f"threshold={watchdog.warning_tokens}",
                    file=sys.stderr,
                    flush=True,
                )
    ...
    with state_lock:
        ...
        observed_tokens = int(watch_state["observed_tokens"])
        largest_delta_bytes = int(watch_state["largest_delta_bytes"])
...
class AutomaticRunner:
    def __init__(self, repo: Path, plan: TaskPlan, progress_path: Path):
        ...
        requested_worker_escalation = (
            os.environ.get("R4R_REQUIRE_SURGICAL_REVIEW", "false").lower()
            == "true"
        )
        if requested_worker_escalation:
            raise ValueError(
                "Worker-side escalation is disabled; request Sol only through Ring ESCALATE"
            )
        ...
        self.opencode_context_delta_bytes = worker_int("CONTEXT_DELTA_BYTES", 65536)
        self.opencode_context_warning_tokens = worker_int(
            "CONTEXT_WARNING_TOKENS", 80000
        )
        self.opencode_context_stop_tokens = worker_int(
            "CONTEXT_STOP_TOKENS", 120000
        )
        ...
        self.escalation_extra_instructions_path = (
            self.control_dir / "ring-extra-instructions.md"
        )
        ...
        self.global_progress_path = (
            global_progress_path(self.ring_worktree)
            if self.ring_worktree is not None
            else None
        )
    ...
    def execute(self) -> int:
        ...
        if assignment is None:
            return self._finish("NO_ACTIVE_ASSIGNMENT", 0)
        assigned_task = self._task_by_id(str(assignment["task_id"]))
        if task_progress(self.progress, assigned_task.id)["status"] == "ACCEPTED":
            return self._finish(
                "ASSIGNMENT_ALREADY_ACCEPTED",
                0,
                {"task": assigned_task.id},
            )
        dirty = git_changed_paths(self.repo)
        selected = assigned_task
        if dirty:
            self._validate_unlocked_resume(dirty, selected)
        result = self._execute_task(assigned_task)
        if result != 0:
            return result
        return self._finish(
            "ASSIGNMENT_COMPLETE",
            0,
            {"task": assigned_task.id, "worker": self.worker_id},
        )
    ...
    def _active_assignment(self) -> dict[str, Any] | None:
        """Return one fresh Ring-generated assignment, or remain quiescent."""
        path = self.ring_directive_path
        ...
        try:
            ...
            return None
        accepted: tuple[str, ...] = ()
        if self.global_progress_path is not None:
            accepted = tuple(
                load_global_progress(self.global_progress_path)["accepted"]
            )
        try:
            return validate_assignment(
                ...
                max_age_seconds=self.ring_directive_max_age_seconds,
            )
        except ValueError as exception:
            if "scope does not match" in str(exception):
    ...
    def _select_task(self) -> Task | None:
        ...
        if assignment is None:
            return None
        task = self._task_by_id(str(assignment["task_id"]))
    ...
    def _execute_task(self, task: Task) -> int:
        ...
        next_action = self._resume_action_from_escalation_extra(task)
        ...
        while attempt_ceiling <= 0 or attempt <= attempt_ceiling:
            ...
            if current_gate.exit_code != 0:
                ...
                next_action = next_action or (
                    ...
                    f"by this evidence: {diagnostics.summary}"
                )
            ...
            if gate.exit_code == 0:
                ...
                self.memory_context["outstanding"] = []
                self.memory_context["next_action"] = (
                    "Preserve gate-green evidence, finalize the task and report "
                    "the result to Ring."
                )
                try:
                ...
                self._notify(
                    attempt_dir,
                    f"handoff-to-escalation-{attempt:02d}",
                    4,
                    f"{task.id}: green gate, handing control to OpenCode escalation",
                )
            else:
                self.memory_context["demonstrated"] = []
                self.memory_context["outstanding"] = [
            ...
            if gate.exit_code != 0:
                next_action = (
                    ...
                    "bounded attempt; request Ring analysis if the task blocks."
                )
            ...
            self.memory_context["outstanding"] = []
            self.memory_context["next_action"] = (
                "Finalize the gate-green task and report global acceptance to Ring."
            )
            self._write_memory()
            self._request_ring_review(
                task,
            ...
            self._write_escalation_extra_instructions(task, {"decision": "ACCEPT"})
        ...
        return self._finish(
            ...
            70,
            {
                ...
                "recovery_packet": str(
                    self.escalation_extra_instructions_path.relative_to(self.repo)
                )
                if self.escalation_extra_instructions_path.exists()
                else None,
            },
        )
    ...
    def _no_progress_action(self, previous_action: str | None) -> str:
        return (
            ...
            "the same unresolved action. Treat the CURRENT ESCALATION-TO-LOCAL EXTRA "
    ...
    def _write_advisory_codegraph_failure(
        ...
        report = (
            ...
            "OpenCode escalation and exact source inspection remain authoritative.\n\n"
            "## Diagnostic\n\n"
            f"{error}\n"
        )
        evidence = attempt_dir / "evidence"
...
"""
    ...
    def _instruction_files(
        ...
        if self.escalation_extra_instructions_path.exists():
            candidates.append(self.escalation_extra_instructions_path)
    ...
    def _use_full_instruction_bundle(self, stage: str) -> bool:
        return (
            ...
            or not self.escalation_extra_instructions_path.exists()
        )
    ...
    def _current_escalation_extra_instructions(self) -> str:
        if not self.escalation_extra_instructions_path.exists():
            return "No active OpenCode escalation-to-local extra instructions."
        return self.escalation_extra_instructions_path.read_text(encoding="utf-8").strip()
    ...
    def _current_ring_directive(self, task: Task) -> str:
        ...
        return (
            ...
            f"Evidence paths:\n{evidence_lines}\n"
            f"Constraints:\n{constraint_lines}\n"
            "Authority: advisory only; the exact task, gate and current OpenCode escalation corrections override it."
        )

    def _resume_action_from_escalation_extra(self, task: Task) -> str | None:
        if not self.escalation_extra_instructions_path.exists():
            return None
        content = self.escalation_extra_instructions_path.read_text(encoding="utf-8")
        if f"- Active task: `{task.id}`" not in content:
            return None
        return (
            "Resume the unfinished revision by applying the complete CURRENT "
            "ESCALATION-TO-LOCAL EXTRA INSTRUCTIONS included below. Do not wait for "
            "another OpenCode escalation review before making the requested correction."
        )
...
"""
    ...
    def _opencode_prompt(
        ...
        extra_instructions = self._current_escalation_extra_instructions()
        ring_directive = self._current_ring_directive(task)
...
OpenCode escalation plan:
{action}

The-Ring advisory directive:
{ring_directive}

Precedence: the exact task, exact gate and OpenCode escalation plan override The-Ring.
...
ESCALATION PLAN OR REVISION ACTION:
{action}

CURRENT ESCALATION-TO-LOCAL EXTRA INSTRUCTIONS:
{extra_instructions}
...
The exact task, deterministic gate and current OpenCode escalation correction packet override The-Ring.
...
ESCALATION ERROR BUNDLE: {diagnostics.bundle_path}
IMPLICATED SOURCE PATHS:
...
OpenCode escalation has received the complete untruncated Maven output and packaged source files.
...
Before editing, translate every numbered OpenCode escalation instruction into an explicit code/test
...
follow OpenCode escalation and exact source/compiler evidence instead of stopping the workflow.
...
## Questions or corrections requested from OpenCode escalation

Be specific. For every numbered OpenCode escalation instruction, state the exact file, method and
...
is sent directly to OpenCode escalation so it can identify your misunderstandings and correct the
...
"""
    ...
    def _write_local_understanding(
        ...
        if not report:
            report = (
                "# Local understanding report\n\n"
                "The local worker produced no model-authored text. OpenCode escalation must treat "
    ...
    def _write_compact_local_understanding(
        ...
        if "# Local understanding report" not in report:
            ...
            report = (
                ...
                "summary. OpenCode escalation must inspect the exact diff.\n\n"
                "## Mapping from requirements to changed code and tests\n"
                f"{changed_text}\n\n"
                "## Claims supported by current gate evidence\n"
        ...
        report += (
            ...
            "OpenCode escalation must still review the diff and acceptance contract.\n"
        )
        (evidence / "local-understanding.md").write_text(
            report.rstrip() + "\n",
            encoding="utf-8",
    ...
    def _write_failed_local_understanding(
        ...
        (evidence / "local-understanding.md").write_text(
            ...
            "OpenCode escalation must review the implementation and gate evidence directly.\n\n"
            "## Diagnostic tail\n"
            f"{diagnostic or 'No diagnostic text was produced.'}\n",
            encoding="utf-8",
        )

    def _write_escalation_extra_instructions(
        self,
        task: Task,
        review: dict[str, Any],
    ) -> None:
        """Persist OpenCode escalation's correction packet outside the Git worktree evidence."""
        self.control_dir.mkdir(parents=True, exist_ok=True)
        if review["decision"] == "ACCEPT":
            self.escalation_extra_instructions_path.unlink(missing_ok=True)
            return
        ...
        content = f"""# OpenCode escalation-to-worker extra instructions

- Generated at: {datetime.now(timezone.utc).isoformat()}
- Active task: `{task.id}`
- OpenCode escalation decision: `{review['decision']}`
...
## OpenCode escalation assessment of the local understanding
...
"""
        temporary = self.escalation_extra_instructions_path.with_suffix(".md.tmp")
        temporary.write_text(content.rstrip() + "\n", encoding="utf-8")
        temporary.replace(self.escalation_extra_instructions_path)
    ...
    def _run_logged(
        ...
        if name == "opencode" or name.startswith("opencode-"):
            watchdog = CommandWatchdog(
                ...
                max_delta_bytes=self.opencode_context_delta_bytes,
                warning_tokens=self.opencode_context_warning_tokens,
                stop_tokens=self.opencode_context_stop_tokens,
            )
        result = run_command(
            command,
            self.repo,
        ...
        (evidence / f"{name}.json").write_text(json.dumps({
            ...
            "largest_delta_bytes": result.largest_delta_bytes,
    ...
    def _finalize_accepted_task(self, task: Task) -> int:
        ...
        # A local commit is not global acceptance. First prove that the shared
        # integration worktree contains this commit and any peer commits needed by
        # the final gate. Leaving local progress non-accepted makes a failed sync
        # safely retryable with the same Ring assignment.
        if not self._sync_with_integration("checkpoint", required=True):
            return self._finish(
                "INTEGRATION_SYNC_REQUIRED",
                69,
                {"task": task.id, "commit": committed_head},
            )
        ledger: dict[str, Any] | None = None
        if self.global_progress_path is not None:
            ledger = record_global_acceptance(
                self.global_progress_path,
                task_id=task.id,
                worker=self.worker_id,
        ...
        if ledger is not None:
            accepted = set(ledger["accepted"])
            if accepted.issuperset(item.id for item in self.plan.tasks):
                final = self._run_gate(
                    "final-gate",
                    self.plan.final_gate,
                    self.run_dir,
                    stream=True,
                )
                record_final_gate(
                ...
                if final.exit_code != 0:
                    self._request_ring_review(
                        task,
                        reason="final-gate-red",
                        attempt=int(
                            task_progress(self.progress, task.id).get(
                                "attempts_total"
                            )
                            or 0
                        ),
                        gate=final,
                        checkpoint_head=committed_head,
                    )
                    return self._finish(
                        "FINAL_GATE_FAILED",
                        final.exit_code,
    ...
    def _repair_trailing_whitespace(self, task: Task) -> list[str]:
        ...
        unstaged_check = run_command(("git", "diff", "--check"), self.repo)
        staged_check = run_command(("git", "diff", "--cached", "--check"), self.repo)

        def candidates(result: CommandResult) -> set[str]:
        ...
        unstaged_candidates = candidates(unstaged_check)
        staged_candidates = candidates(staged_check)
        repaired: set[str] = set()

        # Repair the index directly. `git add` would also stage unrelated working
        # tree edits when a file is only partially staged.
        for relative in sorted(staged_candidates):
            if not path_is_allowed(relative, task.allowed_paths):
                continue
            index_entry = run_command(("git", "ls-files", "-s", "--", relative), self.repo)
            if index_entry.exit_code != 0 or not index_entry.stdout.strip():
                continue
            mode = index_entry.stdout.split(maxsplit=1)[0]
            indexed = run_command(("git", "show", f":{relative}"), self.repo)
            if indexed.exit_code != 0:
                raise RuntimeError(
                    indexed.stderr.strip() or f"Unable to read staged path {relative}"
                )
            normalized = "\n".join(
                line.rstrip(" \t") for line in indexed.stdout.split("\n")
            )
            if normalized == indexed.stdout:
                continue
            blob = run_command(
            ...
            if blob.exit_code != 0 or not blob.stdout.strip():
                raise RuntimeError(blob.stderr.strip() or "Unable to write repaired blob")
            update = run_command(
                (
                    "git",
                    "update-index",
                    "--cacheinfo",
                    f"{mode},{blob.stdout.strip()},{relative}",
                ),
                self.repo,
            )
            if update.exit_code != 0:
                raise RuntimeError(
                    update.stderr.strip() or f"Unable to update staged path {relative}"
                )
            repaired.add(relative)

        # Repair only the working-tree copy for unstaged diagnostics.
        for relative in sorted(unstaged_candidates):
            if not path_is_allowed(relative, task.allowed_paths):
                continue
            path = self.repo / relative
    ...
    def _request_ring_review(
        ...
        *,
        ...
        payload = {
            ...
            "escalation_decision": review.get("decision") if review else None,
    ...
    def _write_memory(self) -> None:
        ...
        lines = [
            ...
            f"- Latest OpenCode escalation decision: {context.get('escalation_decision') or 'pending'}.",
        ...
        if not outstanding:
            lines.append("- Awaiting the next OpenCode escalation decision or exact gate evidence.")
        ...
        lines.extend([
            ...
            "## Fixed decisions",
            "",
            "- OpenCode workers and OpenCode escalation never write Git history.",
            ...
            "- Every red gate retains complete diagnostics for OpenCode escalation.",
